...
const gravatar = require('gravatar'); // Додано імпорт бібліотеки Gravatar
...
router.post('/register', async (req, res) => {
  ...
  try {
    // Отримати email та пароль з тіла запиту
    // eslint-disable-next-line no-unused-vars
    const { email, password } = req.body;

    // Створити URL аватара за допомогою Gravatar
    const avatarURL = gravatar.url(email, { s: '200', r: 'pg', d: 'mm' });

    // Виконати реєстрацію користувача з урахуванням avatarURL
    const result = await registerUser({ ...req.body, avatarURL });
